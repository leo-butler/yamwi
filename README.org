...
- ~$cat_cmd~ :: The pathname of the ~cat~ command. Defaults to ~/usr/bin/cat~.

- ~$git_info~ :: Release or commit information. Default is set by the ~make-tar-gz~ script.
...
- *New*: The Maxima evaluator inspects the code for any *prohibited symbols*, before evaluating it. See the variable =*prohibited-symbols*= in =yamwi.lisp=.
...
*Yamwi* ([[https://github.com/leo-butler/yamwi/][Yet Another Maxima Web Interface]]) is a web interface to the
computer algebra system [[https://maxima.sourceforge.io/][Maxima]]. It is based on based on
Linux+Apache+Php. Originally developed by Mario Rodríguez Riotorto and
hosted on Sourceforge, the project is being continued by a new team of
developers on Github.

*** Important Note
This release contains a [[*New prohibited words][critical security fix]]. Site administrators are advised to update to this release immediately.

*** Security
**** New prohibited words
- ~eval_lisp_string~: this function allows execution of arbitrary code (see [[https://github.com/leo-butler/yamwi/issues/19][Issue 19]]).

**** Formerly prohibited words
- ~kill~ is no longer a prohibited word. The functionality of ~kill~ has been restricted, so ~kill(all)~ and ~kill(allbut(...))~ elicit warnings.
  Killing variables that hold important state information for Yamwi is also prohibited (see [[https://github.com/leo-butler/yamwi/issues/16][Issue 16]] and [[https://github.com/leo-butler/yamwi/commit/daf5d7b40f3a62af03b62dd171bb5a4c3e4a57f9][Commit daf5d7b40f]]).
- ~demo~ is no longer a prohibited word. It is now a synomym for ~batch~, since Yamwi does not allow for line-by-line interaction (see [[https://github.com/leo-butler/yamwi/issues/15][Issue 15]]).
- ~load(drawdf);~ works as intended; ~load~ is no longer a prohibited word (see  [[https://github.com/leo-butler/yamwi/issues/1][Issue 1]]).
- ~concat~, ~sconcat~ and ~printf~ are no longer prohibited (see [[https://github.com/leo-butler/yamwi/issues/18][Issue 18]]).
...
4. Correct the TeX code for matrices and ~diff~ (see [[https://github.com/leo-butler/yamwi/issues/6][Issue 6]] and [[https://github.com/leo-butler/yamwi/issues/6][Issue 11]]).
5. Print the Maxima banner as a header for the output, similar to other front-ends.
6. Maxima's ~trace~ command now works, subject to a patch to Maxima (see [[https://github.com/leo-butler/yamwi/issues/17][Issue 17]]).
7. Commands like ~integrate(x^n,x);~, which require an answer to the question =Is n equal to -1?=, can now have the answer embedded in the input (see [[https://github.com/leo-butler/yamwi/issues/12][Issue 12]]).
8. Commands line ~read("input a number:");~ now have their prompt (=input a number=) echoed correctly. The answer can be placed on the next input line and it is read correctly (see [[https://github.com/leo-butler/yamwi/issues/13][Issue 13]]).
...
12. Yamwi should generate fully compliant HTML that validates with the W3C validator. If you find a page that does not validate, please report a [[https://github.com/leo-butler/yamwi/issues][bug]] and be sure to include a link to non-validating page.
