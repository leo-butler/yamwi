...
- =$mode= ::  Can be one of 0, 1, 2, 3 or 4. These options are:
  0. [@0] When 0, Yamwi output is rendered in ASCII Art style (in command-line ~maxima~, this is called ~1d~ output);
  ...
  2. when 2, results are rendered in enhanced ASCII-Art style, which is easier to read than mode 0 (in command-line ~maxima~, this is ~2d~ output);
  3. when 3, output is returned as a syntactical Maxima expression;
